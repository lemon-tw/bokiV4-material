...
export const getIconButtonStyles = (theme, color = "primary") => {
...
export const getToggleIconButtonStyles = (theme, color = "primary") => {
    const fallback = defaultPalette[color];
    const colorSet = theme?.palette?.[color] ?? fallback;
    return {
        disabledcolor: disableColor[0],
...
const StyleIconButton = styled(MuiIconButton)(({ "data-variant": variant, "data-color": color, size = "small", theme, }) => {
    const styleConfig = getIconButtonStyles(theme, color);
    const safeVariant = ["filled", "standard", "tonal", "outlined"].includes(variant)
        ? variant
        : "filled";
...
const StyleToggleIconButton = styled(MuiIconButton)(({ "data-variant": variant, "data-color": color, clicked: selected, size = "small", theme, }) => {
    ...
    switch (safeVariant) {
        ...
        case "tonal":
            return {
                ...
                "&.Mui-disabled": {
                    backgroundColor: alpha(styleConfig.disabledcolor, 0.08),
                    color: alpha(selected ? current.selectedFontColor : current.fontColor, 0.38),
                },
            };
        case "outlined":
            return {
...
export const IconButton = ({ variant, toggleIcon, children, color = "primary", ...rest }) => {
    ...
    if (toggleIcon) {
        return (_jsx(StyleToggleIconButton, { disableRipple: true, "data-color": color, "data-variant": variant, ...rest, onClick: toggleSelect, clicked: isSelected, children: isSelected ? children : toggleIcon }));
    }
    else {
        return (_jsx(StyleIconButton, { disableRipple: true, "data-color": color, "data-variant": variant, ...rest, children: children }));
    }
};
