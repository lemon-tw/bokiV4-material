...
export const CheckboxTree = ({ data, checkedKeys = [], onChecked, }) => {
    // 處理節點勾選狀態變化
    ...
    const renderTree = (nodes) => {
        return nodes.map(node => (_jsxs("div", { style: { marginLeft: '24px' }, children: [_jsx(Checkbox, { label: node.label, checked: checkedKeys.includes(node.id), indeterminate: isIndeterminate(node), onChange: (e) => handleCheck(node, e.target.checked) }), node.children && renderTree(node.children)] }, node.id)));
    };
    return _jsx("div", { children: renderTree(data) });
};
